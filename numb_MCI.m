...
% Function to display the MCI in Numerical Form
...
disp('CALCULATOR OF MCI IN NUMERIC FORM [hint: angles in rad][use symbols d and q]');
...
switch n
    ...
    case 2
        ...
        % Calculating MCI
        Mci_symbol(x, y, z, variables, numeric_values);   
...
% Function for MCD
function Mci_symbol(x, y, z, variables, numeric_values)
    ...
    disp(J);
    % Numeric Jacobian
    J_numeric = subs(J, variables, numeric_values);
    ...
    disp(det(J_numeric)); % Determinant of Jacobian
    ...
    if det(J_numeric) == 0
        ...
        neg_J = inv(J_numeric); % Inverse Jacobian 
        disp(neg_J);
        ...
        disp(vpa(q1,3)); % Display q1
        q2 = mci(2,1)+mci(2,2)+mci(2,3);
        disp(vpa(q2,3)); % Display q2
        q3 = mci(3,1)+mci(3,2)+mci(3,3);
        disp(vpa(q3,3)); % Display q3

        % doing the total replacement of the speed to find angles
        velocity = input('Enter the velocities of in X, Y and Z in [] EXAMPLE [2 5 8]:');
        double(subs([q1 q2 q3], v, velocity))
